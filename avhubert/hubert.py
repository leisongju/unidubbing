...
class AVHubertConfig(FairseqDataclass):
    ...
    multispkr: bool = field(
        default=False, metadata={"help": "whether use speaker embbding"},
    )
...
class AVHubertModel(BaseFairseqModel):
    ...
    def build_model(cls, cfg: AVHubertConfig, task: AVHubertPretrainingTask):
        ...
        # # 加载 checkpoint
        # checkpoint_path = '/mnt/data/lsj/digital/lrs3/checkpoint/large_vox_iter5.pt'
        # checkpoint = torch.load(checkpoint_path, map_location='cpu')

        # # 修改label_embs_concat层的权重
        # checkpoint['model']['label_embs_concat'] = checkpoint['model']['label_embs_concat'][:1004, :]
    ...
    def forward(
        ...
        if mask and self.masking_type == 'input':
            if src_audio is not None and src_video is None:
                ...
                mask_indices = mask_indices_video
        ...
        if padding_mask is not None:
            ...
            # print("padding_mask after forward padding", padding_mask)
            # exit()
...
@register_model("av_hubert_ReVISE", dataclass=AVHubertConfig)
class AVHubertReVISEModel(BaseFairseqModel):
    def __init__(
        self,
        cfg: AVHubertConfig,
        task_cfg: AVHubertPretrainingConfig,
        ...
        **kwargs
        ...
        self.final_proj = nn.Sequential(
            nn.ConvTranspose1d(in_channels=cfg.encoder_embed_dim, out_channels=final_dim, kernel_size=5, stride=1, padding=2, output_padding=0),
            nn.GELU()
        )
        # modules below are not needed during fine-tuning
        if any([d is None for d in dictionaries]):
    ...
    def build_model(cls, cfg: AVHubertConfig, task: AVHubertPretrainingTask):
        ...
        kwargs = {}
        model = AVHubertReVISEModel(cfg, task.cfg, task.dictionaries, **kwargs)
        # # 加载 checkpoint
        # checkpoint_path = '/mnt/data/lsj/digital/lrs3/checkpoint/large_vox_iter5.pt'
        # checkpoint = torch.load(checkpoint_path, map_location='cpu')
    ...
    def compute_logits(self, feats, emb_mat):
        ...
        if self.sim_type == 'dot':
            ...
            # print("feats ",feats.shape)
            # feats_ = feats.view(-1, emb_dim)
            feats_ = feats.reshape(-1, emb_dim)
            # print("feats_ ", feats_)
            # exit()
            nom = (feats_.unsqueeze(dim=1) * emb_mat.unsqueeze(dim=0)).sum(dim=-1) # [B*T, V]
            denom = (feats_**2).sum(dim=-1).sqrt().unsqueeze(dim=1) * (emb_mat**2).sum(dim=-1).sqrt().unsqueeze(dim=0) # [B*T, V]
...
@register_model("av_hubert_Hifi_nomask", dataclass=AVHubertConfig)
class AVHuberHiFiModel(BaseFairseqModel):
    def __init__(
        self,
        cfg: AVHubertConfig,
        task_cfg: AVHubertPretrainingConfig,
        ...
        **kwargs
        ...
        self.multispkr = cfg.multispkr

        if self.multispkr:
            self.spkr = nn.Embedding(cfg.get("num_speakers", 5600), cfg["encoder_embed_dim"])
        ...
        # else:
        #     self.final_proj = nn.Linear(cfg.encoder_embed_dim, final_dim)
        if self.feat2tar_ratio == 1:
            self.final_proj = nn.Sequential(
            nn.ConvTranspose1d(in_channels=cfg.encoder_embed_dim, out_channels=final_dim, kernel_size=5, stride=1, padding=2, output_padding=0),
            nn.GELU()
            )
        elif self.feat2tar_ratio == 2:
            self.final_proj = nn.Sequential(
            nn.ConvTranspose1d(in_channels=cfg.encoder_embed_dim, out_channels=final_dim, kernel_size=4, stride=2, padding=1, output_padding=0),
            nn.GELU()
            )
        else:
            self.final_proj = nn.Sequential(
                nn.ConvTranspose1d(in_channels=cfg.encoder_embed_dim, out_channels=int(cfg.encoder_embed_dim/2), kernel_size=4, stride=2, padding=1, output_padding=0),
                nn.GELU(),
                nn.ConvTranspose1d(in_channels=int(cfg.encoder_embed_dim/2), out_channels=int(cfg.encoder_embed_dim/2), kernel_size=4, stride=2, padding=1, output_padding=0),
                nn.GELU(),
                nn.ConvTranspose1d(in_channels=int(cfg.encoder_embed_dim/2), out_channels=final_dim, kernel_size=4, stride=2, padding=1, output_padding=0),
                nn.GELU()
            )
            # self.final_proj = nn.Sequential(
            #     nn.ConvTranspose1d(in_channels=cfg.encoder_embed_dim, out_channels=final_dim, kernel_size=8, stride=8, padding=0, output_padding=0),
            #     nn.GELU()
            # )
    ...
    def build_model(cls, cfg: AVHubertConfig, task: AVHubertPretrainingTask):
        ...
        kwargs = {}
        model = AVHuberHiFiModel(cfg, task.cfg, task.dictionaries, **kwargs)
        return model
    ...
    def forward_targets(
        ...
        if self.feat2tar_ratio * feat_tsz > targ_tsz:
            ...
            if mask_indices is not None:
                mask_indices = mask_indices[..., :feat_tsz]
        target_inds = torch.arange(feat_tsz * self.feat2tar_ratio).float()
        target_list = [t[:, target_inds.long()] for t in target_list]
        return features, mask_indices, target_list
    ...
    def forward(
        ...
        if mask_indices is not None:
            ...
            mask = torch.tensor([], dtype=torch.bool)  # 创建一个空的布尔张量
            unmask = torch.logical_not(padding_mask).view(-1)
            unmask = torch.repeat_interleave(unmask, int(self.feat2tar_ratio))
