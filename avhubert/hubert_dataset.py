...
def parse_speaker(path):
    parts = path.rsplit('/', 2)
    new_part = '/spk.list'
    new_path = parts[0] + '/' + parts[1] + new_part
    with open(new_path, 'r') as file:
        speakers = file.read().split('\t')
    return speakers

# def get_spkr_item(fid):
...
class AVHubertDataset(FairseqDataset):
    def __init__(
        ...
        # print("self.audio_root, self.names, inds, tot, self.sizes",len(self.audio_root), len(self.names), len(inds), tot, len(self.sizes))
        # exit()
        self.multispkr = multispkr
        if self.multispkr:
            spkrs = parse_speaker(label_paths[0])
            spkrs.sort()
    ...
    def load_feature(self, mix_name):
        ...
        if 'audio' in self.modalities:
            ...
            wavfile.write(audio_fn.replace('.wav','_noise.wav'), sample_rate, wav_data)
            ...
            #         audio_feats = np.load(
            #             audio_fn.replace('.wav', f'_noise{noise_id}.npy').replace('.ogg', f'_noise{noise_id}.npy'))
            #     else:
            #         audio_feats = np.load(audio_fn.replace('.wav', '.npy').replace('.ogg', '.npy'), allow_pickle=True)
    ...
    def select_noise(self):
        ...
        # rand_indexes = [2700]
        # print("rand_indexes is ",rand_indexes)
        noise_wav = []
    ...
    def __getitem__(self, index):
        ...
        if self.multispkr:
            return {"id": index, 'fid': fid, "video_source": video_feats, 'audio_source': audio_feats, "label_list": labels,"spkr": self._get_spkr(fid)}
        else:
            return {"id": index, 'fid': fid, "video_source": video_feats, 'audio_source': audio_feats, "label_list": labels}
        
    def _get_spkr(self, fid):
        spkr_name = fid.split('/')[-2]
    ...
    def collater(self, samples):
        ...
        # print("self.single_target",self.single_target)
        # exit()
        return batch
