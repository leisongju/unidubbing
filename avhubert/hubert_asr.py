...
from fairseq.models import BaseFairseqModel, FairseqEncoder, FairseqEncoderDecoderModel, register_model, FairseqEncoderModel
...
if DBG:
    ...
    from hubert import AVHubertReVISEModel
    from hubert import AVHuberHiFiModel
...
@register_model("av_hubert_noseq2seq", dataclass=AVHubertSeq2SeqConfig)
class AVHubertnoSeq2Seq(FairseqEncoderModel):
    def __init__(self, encoder, tgt_dict, cfg):
        super().__init__(encoder)
        self.cfg = cfg
        self.freeze_finetune_updates = cfg.freeze_finetune_updates
        self.final_proj_cnn = nn.Sequential(
            nn.ConvTranspose1d(in_channels=1024, out_channels=256, kernel_size=7, stride=1, padding=3, output_padding=0),
            nn.GELU()
        )
    ...
    def build_model(cls, cfg, task):
        ...
        return AVHubertnoSeq2Seq(encoder, tgt_dict, cfg)
    ...
    def forward(self, **kwargs):
        ...
        # decoder_out = self.decoder(prev_output_tokens=kwargs['prev_output_tokens'], encoder_out=output)
        # print(output['encoder_out'].shape)
        # exit()
        # return output
        # 应用卷积层，假设 encoder_out['encoder_out'] 的形状为 T x B x C
        conv_input = encoder_out['encoder_out'].transpose(0, 1).transpose(1, 2)  # 转换形状为 B x C x T
        conv_output = self.final_proj_cnn(conv_input)
        conv_output = conv_output.transpose(1, 2).transpose(0, 1)  # 将形状转换回 T x B x C
        return {
            "encoder_out": conv_output,  # 更新 encoder_out 为卷积后的输出
            "encoder_padding_mask": encoder_out['encoder_padding_mask'],  # 保持 padding_mask 不变
        }
